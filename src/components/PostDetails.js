...
const PostDetails = () => {
  ...
  useEffect(() => {
    // 👉 Paywall function igomba kubanza kuba defined
    const handlePaywall = async (postData, username) => {
      try {
        ...
        if (!depositerSnap.exists()) {
          alert('Account yawe ntabwo tuyibona mubaguze NeS. Tugiye kukujyana aho uzigurira. ubanze uzigure. niba ubusobanuro cyangwa ubufasha, twandikire Whatsapp 0722319367.');
          navigate('/balance');
          return;
        }
        ...
        if (currentNes < 1) {
          alert('Nta NeS ufite zihagije zo gusoma iyi nkuru. Tugiye kukujyana aho uzigurira. Niba ibi ubibonye waziguze, twandikire Whatsapp tugufashe cyangwa tuguhe ubusobanuro bwose ukeneye 0722319367.');
          navigate('/balance');
          return;
        }
    ...
    // 👉 Function yo gufata post & comments
