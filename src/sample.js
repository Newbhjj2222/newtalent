...
const PostDetails = () => {
  ...
  const extractSeriesAndEpisode = (head) => {
    const match = head.match(/^(.*)\s+EP(\d+)$/i);
    if (!match) return { title: null, episode: null };
    return {
  ...
  useEffect(() => {
    ...
    const findAdjacentPosts = async (currentPost) => {
      try {
        ...
        console.log("➡️ Navigation series:", current.title);
        console.log("Current episode:", current.episode);
        console.log("Series posts:", sameSeriesPosts.map(p => `${p.head} (${p.id})`));
      } catch (error) {
    ...
    const handlePaywall = async (postData, username) => {
      try {
        ...
        if (!depositerSnap.exists()) {
          alert('Account yawe ntabwo tubona. Gura NeS point.');
          navigate('/balance');
          return;
        }
        ...
        if (currentNes < 1) {
          alert('Nta NeS point zihagije. Tugiye kukujyana aho uzigurira.');
          navigate('/balance');
          return;
        }
